...
namespace SIMProx
{
    public class ServiceHost : ServiceHostBase
    {
        ...
        protected override void ServiceStartingHandler(object sender, EventArgs<string[]> e)
        {
            ...
            // Make sure SIMProx specific default service settings exist
            CategorizedSettingsElementCollection systemSettings = ConfigurationFile.Current.Settings["systemSettings"];

            systemSettings.Add("CompanyName", "Grid Protection Alliance", "The name of the company who owns this instance of the SIMProx.");
            systemSettings.Add("CompanyAcronym", "GPA", "The acronym representing the company who owns this instance of the SIMProx.");
        }
        ...
        private void SetupRequiredAdapters()
        {
            try
            {
                const string SnmpHostProcessName = "SNMPHOST!PROCESS";
                ...
                using (AdoDataConnection connection = new AdoDataConnection("systemSettings"))
                {
                    ...
                    // Make sure Grafana process adapter exists
                    CustomActionAdapter actionAdapter = actionAdapterTable.QueryRecordWhere("AdapterName = {0}", SnmpHostProcessName) ?? actionAdapterTable.NewRecord();

                    // Update record fields
                    actionAdapter.NodeID = nodeID;
                    actionAdapter.AdapterName = SnmpHostProcessName;
                    actionAdapter.AssemblyName = $"{nameof(SIMProx)}.exe";
                    actionAdapter.TypeName = $"{nameof(SIMProx)}.{nameof(SnmpHost)}";
                    ...
                    if (string.IsNullOrWhiteSpace(actionAdapter.ConnectionString))
                        actionAdapter.ConnectionString = "ConfigFileName=simprox.xml";
